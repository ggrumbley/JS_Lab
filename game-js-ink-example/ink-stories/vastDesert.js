export const storyContent = {
  inkVersion: 19,
  root: [
    [
      '^The desert was vast. My steps were erased soon after I placed them, but I had to continue. I knew that with every movement on the map, I would drink water -- and I was quickly running out.',
      '\n',
      { '->': 'Choices' },
      ['done', { '#f': 5, '#n': 'g-0' }],
      null,
    ],
    'done',
    {
      Choices: [
        [
          'ev',
          { 'VAR?': 'water' },
          1,
          '<',
          '/ev',
          [
            { '->': '.^.b', c: true },
            { b: ['^ ', { '->': 'Out_Of_Water' }, { '->': '.^.^.^.6' }, null] },
          ],
          'nop',
          '\n',
          [
            'ev',
            { '^->': 'Choices.0.8.$r1' },
            { 'temp=': '$r' },
            'str',
            { '->': '.^.s' },
            [{ '#n': '$r1' }],
            '/str',
            '/ev',
            { '*': '.^.^.c-0', flg: 2 },
            { s: ['^Move north?', { '->': '$r', var: true }, null] },
          ],
          [
            'ev',
            { '^->': 'Choices.0.9.$r1' },
            { 'temp=': '$r' },
            'str',
            { '->': '.^.s' },
            [{ '#n': '$r1' }],
            '/str',
            '/ev',
            { '*': '.^.^.c-1', flg: 2 },
            { s: ['^Move south?', { '->': '$r', var: true }, null] },
          ],
          [
            'ev',
            { '^->': 'Choices.0.10.$r1' },
            { 'temp=': '$r' },
            'str',
            { '->': '.^.s' },
            [{ '#n': '$r1' }],
            '/str',
            '/ev',
            { '*': '.^.^.c-2', flg: 2 },
            { s: ['^Move west?', { '->': '$r', var: true }, null] },
          ],
          [
            'ev',
            { '^->': 'Choices.0.11.$r1' },
            { 'temp=': '$r' },
            'str',
            { '->': '.^.s' },
            [{ '#n': '$r1' }],
            '/str',
            '/ev',
            { '*': '.^.^.c-3', flg: 2 },
            { s: ['^Move east?', { '->': '$r', var: true }, null] },
          ],
          {
            'c-0': [
              'ev',
              { '^->': 'Choices.0.c-0.$r2' },
              '/ev',
              { 'temp=': '$r' },
              { '->': '.^.^.8.s' },
              [{ '#n': '$r2' }],
              '\n',
              'ev',
              { 'VAR?': 'y' },
              1,
              '-',
              { 'f()': 'setY' },
              'out',
              '/ev',
              '\n',
              'ev',
              { 'f()': 'drinkWater' },
              'out',
              '/ev',
              '\n',
              { '->': '.^.^.^' },
              { '#f': 5 },
            ],
            'c-1': [
              'ev',
              { '^->': 'Choices.0.c-1.$r2' },
              '/ev',
              { 'temp=': '$r' },
              { '->': '.^.^.9.s' },
              [{ '#n': '$r2' }],
              '\n',
              'ev',
              { 'VAR?': 'y' },
              1,
              '+',
              { 'f()': 'setY' },
              'out',
              '/ev',
              '\n',
              'ev',
              { 'f()': 'drinkWater' },
              'out',
              '/ev',
              '\n',
              { '->': '.^.^.^' },
              { '#f': 5 },
            ],
            'c-2': [
              'ev',
              { '^->': 'Choices.0.c-2.$r2' },
              '/ev',
              { 'temp=': '$r' },
              { '->': '.^.^.10.s' },
              [{ '#n': '$r2' }],
              '\n',
              'ev',
              { 'VAR?': 'x' },
              1,
              '-',
              { 'f()': 'setX' },
              'out',
              '/ev',
              '\n',
              'ev',
              { 'f()': 'drinkWater' },
              'out',
              '/ev',
              '\n',
              { '->': '.^.^.^' },
              { '#f': 5 },
            ],
            'c-3': [
              'ev',
              { '^->': 'Choices.0.c-3.$r2' },
              '/ev',
              { 'temp=': '$r' },
              { '->': '.^.^.11.s' },
              [{ '#n': '$r2' }],
              '\n',
              'ev',
              { 'VAR?': 'x' },
              1,
              '+',
              { 'f()': 'setX' },
              'out',
              '/ev',
              '\n',
              'ev',
              { 'f()': 'drinkWater' },
              'out',
              '/ev',
              '\n',
              { '->': '.^.^.^' },
              { '#f': 5 },
            ],
          },
        ],
        { '#f': 1 },
      ],
      Out_Of_Water: [
        '^I shook my canteen, but could tell it was empty. This... this was the last step in my journey.',
        '\n',
        'done',
        { '#f': 1 },
      ],
      setX: [
        { 'temp=': 'newX' },
        [
          'ev',
          { 'VAR?': 'newX' },
          100,
          '>',
          '/ev',
          { '->': '.^.b', c: true },
          { b: ['\n', 'ev', 100, '/ev', { 'VAR=': 'x', re: true }, { '->': 'setX.4' }, null] },
        ],
        [
          'ev',
          { 'VAR?': 'newX' },
          0,
          '<',
          '/ev',
          { '->': '.^.b', c: true },
          { b: ['\n', 'ev', 0, '/ev', { 'VAR=': 'x', re: true }, { '->': 'setX.4' }, null] },
        ],
        [
          { '->': '.^.b' },
          {
            b: [
              '\n',
              'ev',
              { 'VAR?': 'newX' },
              '/ev',
              { 'VAR=': 'x', re: true },
              { '->': 'setX.4' },
              null,
            ],
          },
        ],
        'nop',
        '\n',
        { '#f': 1 },
      ],
      setY: [
        { 'temp=': 'newY' },
        [
          'ev',
          { 'VAR?': 'newY' },
          100,
          '>',
          '/ev',
          { '->': '.^.b', c: true },
          { b: ['\n', 'ev', 100, '/ev', { 'VAR=': 'y', re: true }, { '->': 'setY.4' }, null] },
        ],
        [
          'ev',
          { 'VAR?': 'newY' },
          0,
          '<',
          '/ev',
          { '->': '.^.b', c: true },
          { b: ['\n', 'ev', 0, '/ev', { 'VAR=': 'y', re: true }, { '->': 'setY.4' }, null] },
        ],
        [
          { '->': '.^.b' },
          {
            b: [
              '\n',
              'ev',
              { 'VAR?': 'newY' },
              '/ev',
              { 'VAR=': 'y', re: true },
              { '->': 'setY.4' },
              null,
            ],
          },
        ],
        'nop',
        '\n',
        { '#f': 1 },
      ],
      currentPosition: [
        '^(Currently at ',
        'ev',
        { 'VAR?': 'x' },
        'out',
        '/ev',
        '^ and ',
        'ev',
        { 'VAR?': 'y' },
        'out',
        '/ev',
        '^.)',
        '\n',
        { '#f': 1 },
      ],
      drinkWater: [
        'ev',
        { 'VAR?': 'water' },
        1,
        '-',
        '/ev',
        { 'VAR=': 'water', re: true },
        { '#f': 1 },
      ],
      currentWater: [
        '^(Remaining water ',
        'ev',
        { 'VAR?': 'water' },
        'out',
        '/ev',
        '^)',
        '\n',
        { '#f': 1 },
      ],
      'global decl': [
        'ev',
        20,
        { 'VAR=': 'x' },
        20,
        { 'VAR=': 'y' },
        20,
        { 'VAR=': 'water' },
        '/ev',
        'end',
        null,
      ],
      '#f': 1,
    },
  ],
  listDefs: {},
};
